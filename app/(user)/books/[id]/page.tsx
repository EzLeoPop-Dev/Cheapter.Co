...
import {
  ...
  Star,
  ChevronDown,
  ChevronUp, // 🌟 เพิ่ม ChevronUp สำหรับปุ่มย่อรูป
  Check,
  ChevronLeft,
  ChevronRight,
  Quote,
...
const RELATED_BOOKS = [
  ...
  },
  {
    ...
    price: "$22.00",
    imageUrl:
      "https://images.unsplash.com/photo-1629196914234-a69077ee8478?q=80&w=400&auto=format&fit=crop",
  },
];
...
export default function BookDetailPage() {
  ...
  // State สำหรับจัดการขยาย/พับเก็บรูปภาพ
  const [expandedReviewImages, setExpandedReviewImages] = useState<
    Record<number, boolean>
  >({});

  const toggleReviewImages = (reviewId: number) => {
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-8 py-16">
        ...
        <div className="flex flex-col md:flex-row gap-16 lg:gap-24 mb-32">
          ...
          <div className="w-full md:w-3/5 flex flex-col pt-4 md:pl-8">
            ...
            >
              ...
              <ChevronDown
                size={12}
                className={`transform transition-transform ${isReadMore ? "rotate-180" : ""}`}
              />
            </button>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <Heart
                  size={14}
                  className={isWishlisted ? "fill-[#b46b45]" : ""}
                />
        ...
        {BOOK.quote && (
          <div className="w-full py-12 mb-20 relative border-t border-stone-200/50">
            <Quote
              className="absolute top-4 left-0 text-[#e6dbcc] opacity-50 rotate-180"
              size={36}
            />
            <div className="px-12 z-10 relative">
              <p
                className={`text-xl md:text-2xl text-left text-stone-700 font-serif leading-relaxed ${isQuoteExpanded ? "" : "overflow-hidden [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:3]"}`}
              >
                {BOOK.quote}
              </p>
              <button
              ...
              >
                ...
                <ChevronDown
                  size={12}
                  className={`transform transition-transform ${isQuoteExpanded ? "rotate-180" : ""}`}
                />
              </button>
            </div>
            <Quote
              className="absolute bottom-4 right-0 text-[#e6dbcc] opacity-50"
              size={36}
            />
          </div>
        )}
        ...
        <div
          className={`w-full max-w-4xl mx-auto py-12 mb-20 ${!BOOK.quote ? "border-t border-stone-200/50" : ""}`}
        >
          <h2 className="text-xl font-bold text-stone-800 font-serif mb-6 tracking-tight">
            รายละเอียดหนังสือ (Product Details)
          </h2>
        ...
        <div className="w-full pt-16 border-t border-stone-200/50 mb-16">
          <h2 className="text-2xl font-bold text-stone-800 font-serif tracking-tight mb-8">
            รีวิวจากผู้อ่าน (Customer Reviews)
          </h2>
          ...
          <div className="flex flex-col md:flex-row gap-12">
              ...
              {reviewStats && reviewStats.totalReviews > 0 ? (
                <>
                  <div className="flex items-end gap-3 mb-6">
                    <span className="text-5xl font-bold text-stone-800 leading-none">
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-1 w-10 shrink-0">
                            <span
                              className={`text-xs font-bold ${isSelected ? "text-[#b46b45]" : "text-stone-600"}`}
                            >
                              {star}
                            </span>
                            <Star
                ...
                <div className="text-stone-500 text-sm">
                  ยังไม่มีรีวิวสำหรับหนังสือเล่มนี้ (No reviews yet)
                </div>
              )}
            </div>
            ...
            <div className="w-full md:w-2/3 flex flex-col gap-6">
              {isLoadingReviews ? (
                ...
                reviews.map((review) => {
                  const { cleanComment, extractedImages } = parseReviewComment(
                    review.comment,
                  );
                  ...
                  return (
                    <div
                      key={review.id}
                      className="bg-white p-5 rounded-lg shadow-sm border border-stone-100"
                    >
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <div className="flex flex-col items-end gap-1">
                          ...
                          <div className="text-[10px] font-medium text-[#b46b45]">
                            ให้คะแนน {review.rating} ดาว
                          </div>
                        </div>
                      </div>

                      {cleanComment && (
                        <p className="text-sm text-stone-600 leading-relaxed font-sans mt-2">
                          {cleanComment}
                        </p>
                      )}

                      {/* 🌟 แสดงรูปภาพรีวิว (แสดงสูงสุด 2 รูป) */}
                      {allImages.length > 0 && (
                        <div className="mt-4 flex flex-wrap gap-2 items-center">
                          {visibleImages.map((imgUrl, index) => {
                            ...
                            return (
                              <div
                                key={index}
                                className="relative w-24 h-24 rounded-lg cursor-pointer group overflow-hidden border border-gray-200"
                                onClick={() => {
                                  if (isLastVisible) {
                              ...
                              >
                                <img
                                  ...
                                  className={`w-full h-full object-cover transition-opacity duration-300 ${isLastVisible ? "opacity-80" : "group-hover:scale-105"}`}
                                />
                                {isLastVisible && (
                                  <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center text-white backdrop-blur-[1px] hover:bg-black/60 transition-colors">
                                    <Images
                                      size={20}
                          ...
                          {/* 🌟 เพิ่มปุ่มสำหรับกด "ย่อรูปภาพ" เมื่ออยู่ในสถานะขยายแล้ว */}
                          {isImagesExpanded &&
                            allImages.length > MAX_VISIBLE_IMAGES && (
                              <button
                                onClick={() => toggleReviewImages(review.id)}
                                className="h-24 px-3 rounded-lg border border-stone-200 bg-stone-50 hover:bg-stone-100 text-stone-600 text-xs font-semibold flex flex-col items-center justify-center gap-1 transition-colors cursor-pointer"
                                title="พับเก็บรูปภาพ"
                              >
                                <ChevronUp size={16} />
                                <span>ย่อรูปภาพ</span>
                ...
                <div className="bg-white p-8 rounded-lg shadow-sm border border-stone-100 text-center flex flex-col items-center justify-center">
                  ...
                  <p className="text-stone-500 text-sm">
                    ไม่มีรีวิวที่ตรงกับตัวกรองของคุณ (No reviews matching your
                    filter)
                  </p>
                </div>
              )}
        ...
        <div className="w-full pt-16 border-t border-stone-200/50">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-base font-bold text-stone-900 font-serif tracking-tight">
              You might also like
            </h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 xl:grid-cols-6 gap-5">
            {RELATED_BOOKS.map((book) => (
              ...
              >
                ...
                <h3 className="font-bold text-stone-800 text-xs mb-0.5 leading-snug group-hover:text-[#b46b45] transition-colors">
                  {book.title}
                </h3>
                <p className="font-serif italic text-stone-500 text-[10px] mb-1.5">
                  {book.author}
                </p>
                <div className="flex items-center gap-2">
                  <span className="font-sans font-bold text-stone-800 text-xs">
                    {book.price}
                  </span>
                  {book.originalPrice && (
