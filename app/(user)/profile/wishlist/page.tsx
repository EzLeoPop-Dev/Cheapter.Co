...
export default function WishlistPage() {
  const wishlistItems = [
    ...
    },
    {
      ...
      price: '280.00',
      stock: true,
      image: 'https://images.unsplash.com/photo-1535905557558-afc4877a26fc?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80'
    }
  ];
        ...
        {wishlistItems.map((item) => (
          <div key={item.id} className="bg-[#fefdfb] rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow group">
            <div className="relative h-48 w-full bg-gray-100">
              ...
              />
              <button className="absolute top-3 right-3 p-2 bg-white/80 backdrop-blur-sm rounded-full text-red-500 hover:bg-white hover:text-red-600 transition-colors opacity-0 group-hover:opacity-100">
            ...
            <div className="p-5">
              ...
              <div className="flex justify-between items-center mb-4">
                <span className="font-bold text-lg text-[#bc876e]">฿{item.price}</span>
                <span className={`text-xs px-2 py-1 rounded-full ${item.stock ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                  {item.stock ? 'มีสินค้า' : 'สินค้าหมด'}
                </span>
              </div>
              ...
              >
                ...
                {item.stock ? 'หยิบใส่ตะกร้า' : 'สินค้าหมด'}
              </button>
            </div>
          </div>
        ))}
