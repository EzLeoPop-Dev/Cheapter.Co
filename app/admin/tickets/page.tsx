...
export default function AdminTicketsPage() {
    ...
    if (selectedTicket) {
      ...
      const channel = supabase
        ...
        .on('postgres_changes', { 
          ...
          // Also refresh tickets to update timestamps
          fetchTickets();
        })
        .subscribe();
  ...
  const handleSendMessage = async (e: React.FormEvent, closeTicket = false) => {
    e.preventDefault();
    if (!newMessage.trim() && !closeTicket) return;
    ...
    try {
      const res = await fetch(`/api/admin/tickets/${selectedTicket.id}/messages`, {
        ...
        body: JSON.stringify({ 
          message: newMessage || (closeTicket ? 'Ticket closed by Admin' : ''), 
          markAsResolved: closeTicket 
        }),
      });
      const data = await res.json();
      setNewMessage('');
          ...
          <div className="flex-1 bg-white border border-gray-200 rounded-2xl shadow-sm flex flex-col overflow-hidden">
            ...
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <div>
                ...
                <p className="text-xs text-gray-500">Ticket: {selectedTicket.id} • Customer: {selectedTicket.user?.name}</p>
              </div>
