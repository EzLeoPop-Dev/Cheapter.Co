...
export default function AdminPromotionsPage() {
  ...
  const [promoFormData, setPromoFormData] = useState({
    name: '', code: '', discount: '', end_date: '', status: 'Active'
  });
            ...
            <tbody className="divide-y divide-[#e6e5e0]">
              {promotions.map((promo) => (
                <tr key={promo.id} className="hover:bg-white/50 transition-colors">
                  ...
                  <td className="py-4 px-4 text-sm text-[#a09c92]">{promo.end_date}</td>
                  <td className="py-4 px-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setShowAddPromoModal(false)}>
          <div className="bg-[#FDFBF7] rounded-[2rem] w-full max-w-2xl shadow-xl overflow-hidden flex flex-col max-h-[85vh]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 overflow-y-auto flex-1 bg-white space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-[#1A1A1A] mb-1">วันหมดอายุ <span className="text-red-500">*</span></label>
                  <input 
                    type="text" 
                    value={promoFormData.end_date} 
                    onChange={e => setPromoFormData({...promoFormData, end_date: e.target.value})} 
                    placeholder="เช่น 31 ธ.ค. 2026, ไม่มีกำหนด"
                    className="w-full px-3 py-2 border border-[#e6e5e0] rounded-xl focus:outline-none focus:border-indigo-600 text-sm" 
                  />
                </div>
                <div>
            ...
            <div className="p-4 border-t border-[#e6e5e0] bg-white flex justify-end gap-3">
              ...
              <button 
                onClick={() => {
                  if (promoFormData.name && promoFormData.code && promoFormData.discount && promoFormData.end_date) {
                    if (editingPromoId) {
                ...
                className={`px-5 py-2.5 rounded-xl font-bold transition-colors text-sm ${(promoFormData.name && promoFormData.code && promoFormData.discount && promoFormData.end_date) ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
                disabled={!(promoFormData.name && promoFormData.code && promoFormData.discount && promoFormData.end_date)}
              >
                บันทึกโปรโมชั่น
              </button>
            </div>
