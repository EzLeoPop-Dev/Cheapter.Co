...
export default function AdminStockPage() {
  ...
  const openLogDrawer = async (item) => {
    setLogDrawer({ ...item, loading: true, logs: [] });
    try {
      const response = await fetch(`/api/admin/stock-movements?bookId=${item.id}`);
      if (!response.ok) throw new Error('Failed');
      const movements = await response.json();
      const realLogs = movements.map((m: any) => ({
        id: m.id,
        qty: m.type === 'OUT' || m.type === 'DAMAGED' ? -m.quantity : m.quantity,
        balance: m.balanceAfter,
        ref: m.note || m.reference || m.type,
        date: m.createdAt
      }));
      setLogDrawer({ ...item, logs: realLogs, loading: false });
    } catch {
      setLogDrawer({ ...item, logs: [], loading: false });
    }
  };
  ...
  return <div className="space-y-6">
    ...
    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
      ...
      {isLoading ? <div className="h-64 flex items-center justify-center text-gray-500">Loading inventory...</div> : <div className="overflow-x-auto"><table className="w-full text-left"><thead className="text-xs text-gray-400 uppercase bg-gray-50"><tr><th className="p-4">{t('stk.col.product')}</th><th className="p-4">{t('stk.col.category')}</th><th className="p-4 text-center">{t('stk.col.inStock')}</th><th className="p-4 text-center">{t('stk.col.available')}</th><th className="p-4 text-center">ขายแล้ว</th><th className="p-4">{t('usr.col.status')}</th><th className="p-4" /></tr></thead><tbody>{filteredStock.map((item) => <tr key={item.id} className="border-t border-gray-100"><td className="p-4"><button onClick={() => openLogDrawer(item)} className="text-left"><b className="block">{item.name}</b><span className="text-xs text-gray-400">#{item.id}</span></button></td><td className="p-4 text-sm text-gray-600">{item.category}</td><td className="p-4 text-center font-bold">{item.current}</td><td className="p-4 text-center font-bold">{item.available}</td><td className="p-4 text-center font-bold text-green-600">{item.sold || 0}</td><td className="p-4"><span className={`inline-flex px-2 py-1 rounded border text-xs font-bold ${statusClass(item.status)}`}>{statusLabel[item.status]}</span></td><td className="p-4 text-right"><button onClick={() => openAdjust(item)} className="px-3 py-1.5 text-xs font-bold border rounded-lg inline-flex gap-1"><Settings2 className="w-3 h-3" />{t('stk.action.add')}</button></td></tr>)}</tbody></table>{filteredStock.length === 0 && <p className="text-center py-12 text-gray-500">No items found</p>}</div>}
    </div>
    ...
    {logDrawer && <div className="fixed inset-0 z-50 flex justify-end bg-gray-900/40" onClick={() => setLogDrawer(null)}><div className="bg-white w-full max-w-lg h-full p-6 overflow-y-auto" onClick={(event) => event.stopPropagation()}><div className="flex justify-between mb-6"><div><h3 className="font-black text-lg">{t('stk.log.title')}</h3><p className="text-sm text-gray-500">{logDrawer.name}</p></div><button onClick={() => setLogDrawer(null)}><X /></button></div>{logDrawer.loading ? <p className="text-center text-gray-500 py-12">Loading history...</p> : logDrawer.logs?.length ? logDrawer.logs.map((log) => <div key={log.id} className="p-4 mb-3 bg-gray-50 rounded-xl border"><div className="flex justify-between"><b className={log.qty >= 0 ? 'text-green-600' : 'text-red-600'}>{log.qty >= 0 ? `+${log.qty}` : log.qty}</b><span className="text-xs">Balance: {log.balance}</span></div><p className="text-sm mt-1">{log.ref}</p><p className="text-xs text-gray-400 mt-2">{new Date(log.date).toLocaleString('th-TH')}</p></div>) : <p className="text-center text-gray-500 py-12">No movement history</p>}</div></div>}
  </div>;
}
